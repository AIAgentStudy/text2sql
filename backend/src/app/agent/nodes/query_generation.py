...
17. 설명은 반드시 한국어로 작성하며, 쿼리가 어떤 데이터를 조회하는지 요약하세요. 데이터베이스 스키마나 테이블 정의를 설명에 포함하지 마세요.
...
## 출력 형식
다음 형식으로 정확히 응답하세요 (Markdown 코드 블록과 설명):
...
16. 설명은 반드시 한국어로 작성하며, 쿼리가 어떤 데이터를 조회하는지 요약하세요. 데이터베이스 스키마나 테이블 정의를 설명에 포함하지 마세요.
...
def _parse_llm_response(response: str) -> tuple[str, str]:
    ...
    if "설명:" in response:
        try:
            ...
            # 다음 섹션이나 불필요한 마크다운 헤더가 있으면 자르기
            # 예: ## 데이터베이스 스키마, ``` 등이 딸려오는 경우 방지
            cut_indicators = ["```", "##"]
            indices = [explanation.find(ind) for ind in cut_indicators if explanation.find(ind) != -1]
            
            if indices:
                cut_index = min(indices)
                explanation = explanation[:cut_index].strip()
